...
├── utils/                    # 工具函数
├── mask_probe.py             # 掩码组合损失探测
├── mask_analysis.py          # 损失分箱与关联规则分析
├── train.py                  # 训练入口
├── evaluate.py               # 评估入口
...
当前模型在工业制氢 61 通道传感器数据上完成了稀疏观测重建实验。模型规模约 0.39M 参数，主要任务为单点状态预测与历史序列重构。

当前交付版本的实验记录显示，在 `mask_ratio = 0.7` 的高比例稀疏观测条件下，模型仍可保持约 87% 的重构与预测精度。训练日志保存在 `results/` 目录下，模型权重保存在 `checkpoints/` 目录下。

### 11.1 内部聚类（InsideCo）实验

内部聚类实验用于解释 CIST-MAE 学到的跨传感器依赖关系。该实验不直接使用物理拓扑标签，而是从模型在不同掩码组合下的重构损失出发，挖掘“同时被掩码时更容易导致高损失”的传感器组合，从而发现具有较强冗余或互补关系的传感器簇。

实验流程如下：

1. 使用掩码探测结果 `results/0.4mask_probe_results.csv`，其中每条记录包含一组 `mask_vector` 及其平均重构损失 `avg_loss`；
2. 通过 K-Means 将掩码组合按重构损失分为 `S_low`、`S_mid`、`S_high` 三类，并选取高损失样本集合 `S_high`；
...
| 输出文件 | 内容说明 |
|---|---|
| `results/inside_frequent_items.csv` | 高损失样本集合中的频繁项集 |
| `results/inside_rules.csv` | Apriori 关联规则及 Lift 指标 |
| `results/inside_clusters.json` | 基于 Lift 贪心构建的内部聚类结果 |

当前聚类结果共发现 13 个非孤立簇，覆盖 56 个传感器，另有 5 个孤立传感器。主要聚类示例如下：
...
| 实验 | 实验设计 | 输出文件 | 当前结果 |
|---|---|---|---|
| Exp1: 掩码策略扫描 | 比较跨簇分散掩码、簇内集中掩码、随机掩码，掩码率从 0.05 扫描到 0.90 | `results/Exp1_InsideCo_sweep.csv`, `results/Exp1_InsideCo_sweep.png` | 跨簇分散掩码在 69/86 个掩码率上优于簇内集中掩码，表明保留簇内信息对重构结果具有正向影响 |
...
上述结果表明，CIST-MAE 的重构性能不仅依赖可见传感器数量，也受到可见传感器结构位置的影响；在本次实验统计中，同一内部簇中的传感器能够提供更强的替补信息。

---

## 12. 交付产物与结果文件

当前仓库保留了模型训练、掩码探测、内部聚类与验证实验的主要输出文件。核心结果文件如下：

| 文件 | 内容说明 |
|---|---|
| `checkpoints/best.pth` | 当前最优模型权重 |
| `results/0.4mask_probe_results.csv` | 掩码组合损失探测结果 |
| `results/inside_frequent_items.csv` | 高损失样本集合频繁项集 |
| `results/inside_rules.csv` | Apriori 关联规则结果 |
| `results/inside_clusters.json` | 内部聚类划分结果 |
| `results/Exp1_InsideCo_sweep.csv` | 三种掩码策略扫描结果 |
| `results/Exp1_InsideCo_sweep.png` | 三种掩码策略扫描曲线 |
| `results/Exp2_IntraCluster_Support.csv` | 簇内互助受控实验逐传感器结果 |
| `results/Exp2_IntraCluster_Support.png` | 簇内互助与跨簇推断对比图 |
| `results/model_comparison.csv` | 模型对比结果记录 |
| `results/sensor_importance.csv` | 传感器重要性分析结果 |

以上文件共同构成当前版本的实验记录与结果追溯依据。
